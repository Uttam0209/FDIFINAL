...
public partial class Grievance_GAssignTicket : System.Web.UI.Page
{
    #region Variable    
    Logic Lo = new Logic();
    ...
    {
        if (!IsPostBack)
        { BindYear(); BindMonth(); BindType(); bindportal(); BindRecord(); BindUser(); }
    }
    #region MasterData
    ...
    {
        DataTable DtMaster = Lo.RetriveHelpdesk(0, 0, 0, "", "", "", "", "Assignto");
    ...
    {
        DataTable Dtuser = Lo.RetriveHelpdesk(0, 0, 0, "", "", "", "", "UserName");
        if (Dtuser.Rows.Count > 0)
        {
            Co.FillDropdownlist(ddlassign, Dtuser, "Name", "Gid");
    ...
    {
        if (ddlassign.SelectedItem.Text != "Select")
        {

            foreach (GridViewRow row in gvcase.Rows)
            {
                try
                {
                    Label HfID = (Label)row.FindControl("lbrefno") as Label;
                    if ((row.FindControl("chkRow") as CheckBox).Checked)
                    {
                        DataTable UpdateNewFEtoLocation = Lo.RetriveHelpdesk(Convert.ToInt32(ddlassign.SelectedItem.Value), 0, 0, HfID.Text.Trim(), "2", "", "", "AssignHelpdeskjob");
                    }
                }
                catch (Exception ex)
                {
            ...
            string message = new System.Web.Script.Serialization.JavaScriptSerializer().Serialize("Successfully assign job to Helpdesk/Developer/Manager team.");
            string script = string.Format("alert({0});window.location ='AssignTicket';", message);
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "", script, true);
        }
        else
        {
            ScriptManager.RegisterStartupScript(Page, Page.GetType(), "alert", "alert('Select  one Helpdesk/Developer/Manager')", true);
        }
    }
    #endregion
}