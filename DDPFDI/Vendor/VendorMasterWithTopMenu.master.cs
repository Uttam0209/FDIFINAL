...
public partial class Vendor_VendorMasterWithTopMenu : System.Web.UI.MasterPage
{
    Logic Lo = new Logic();
    ...
    private void bindTopMenu(string sType)
    {
        StringBuilder strTopMenu = new StringBuilder();
        string[] TCateg = strMasterAlloted.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
        string MmCval = "";
        for (int x = 0; x < TCateg.Length; x++)
        {
            MmCval = TCateg[x];
            DataTable dtMArea = Lo.RetriveMasterData(Convert.ToInt64(MmCval), sType, "", 0, "", "", "VendorIntAreaMenuId");
            foreach (DataRow row in dtMArea.Rows)
            {
                string[] TCateg1 = dtMArea.Rows[0]["MenuId"].ToString().Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
                string TmCval1 = "";
                for (int j = 0; j < TCateg1.Length; j++)
                {
                    TmCval1 = TCateg1[j];
                    DataTable dtMMenu = Lo.RetriveMasterData(0, "", ObjEnc.DecryptData(Session["Type"].ToString()), Convert.ToInt16(TmCval1), "", "", "MenuMainVendor");
                    foreach (DataRow row2 in dtMMenu.Rows)
                    {
                        DataTable SubMmenu = Lo.RetriveMasterData(0, "", ObjEnc.DecryptData(Session["Type"].ToString()), Convert.ToInt16(row2["MenuID"].ToString()), "", "", "SubMenuVendor");
                        if (SubMmenu.Rows.Count > 0)
                        {
                            foreach (DataRow row1 in SubMmenu.Rows)
                            {
                                strTopMenu.Append("<button class=tablinks ><a href = '" + row1["MenuUrl"].ToString() + "?mu=" + ObjEnc.EncryptData(row1["Spanclass"].ToString()) + "&id=" + ObjEnc.EncryptData(row["InterestArea"].ToString()) + "' class=tablinks>" + row1["MenuName"].ToString() + "</a></button>");
                                // strTopMenu.Append("<button class=tablinks ><a href = '" + row1["MenuUrl"].ToString() + "?mu=" + ObjEnc.EncryptData(row1["Spanclass"].ToString()) + "&id=" + ObjEnc.EncryptData(row1["InterestArea"].ToString()) + "' class=tablinks>" + row1["MenuName"].ToString() + "</a></button>");

                            }

                        }
                        //TopMenu.InnerHtml = strTopMenu.ToString();
    ...
    protected void MenuLogin()
    {
        ...
        if (Session["VendorRefNo"] != null)
        {
            ...
            if (dtCompany.Rows.Count > 0)
            {
                ...
                // Session["CompName"] = dtCompany.Rows[0]["V_CompName"].ToString();
            }
        }
